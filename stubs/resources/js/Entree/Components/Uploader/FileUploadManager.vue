...
let attachment = reactive({ list:[], uploader: { id: props.id, model: props.model } });

const fileUploader = ref(null)

const uniqueId = ref((new Date()).getTime())
...
const removeUpload = async media => {
    ...
    emit('fileRemoved', attachment.list)
}


watchEffect( async () => {
    if (props.id && props.model) {
        attachment.list = await webhook.search('api.attachment.list', attachment.uploader)
        fileUploader.value.fileUploader.form = attachment.uploader
    }
})

watch(() => props.id, (modelId, oldModelId) => {
    attachment.uploader = { id: modelId, model: props.model }
})

watch(() => props.model, (modelClass, oldModelId) => {
    attachment.uploader = { id: props.id, model: modelClass }
})
...
<template>
    <div class="w-full p-2 mt-6 border-2 border-gray-900 dark:border-gray-50">
        ...
        <div class="flex space-x-2">
            <div class="flex-1 grid grid-cols-3 place-items-stretch gap-2 bg-gray-50">
                <template  v-for="(media, uuid) in attachment.list" :key="uuid">
                    <div class="flex items-start group dark:text-gray-900 p-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                             viewBox="0 0 24 24" stroke-width="1.5"
                             stroke="currentColor" class="flex-initial w-6 h-6 mr-2 group-hover:hidden">
                        ...
                        <div class="group-hover:flex-initial hidden group-hover:inline-flex">
                            ...
                            <div v-if="authorize" class="group/edit rounded-full hover:ring-0.5 p-0.5 hover:ring-blue-500 hover:bg-blue-500 flex items-center"
            ...
            <div v-if="authorize && props.id" class="flex-inital items-end">
                <Uploader class="peer p-0"
                          :key="uniqueId"
                          :id="uniqueId"
                          ref="fileUploader"
                          :url="url"
                          :form="attachment.uploader"
